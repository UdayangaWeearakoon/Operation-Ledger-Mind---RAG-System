...
class LLMClient:
    ...
    def chat(
        ...
        expect_json: bool = False,               # NEW: ask for JSON parsing
        **kwargs,
    ) -> Dict[str, Any]:
        """
        Send chat completion request with automatic retry and token management.
        Now supports optional JSON parsing with expect_json=True.
        """
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                # ── JSON parsing when requested ───────────────────────────────
                parsed = None
                parse_error = None
                if expect_json:
                    cleaned_text = text
                    # Remove common markdown code fences
                    if re.match(r'^```json\s*', cleaned_text, re.IGNORECASE):
                        cleaned_text = re.split(r'^```json\s*|\s*```$', cleaned_text, flags=re.IGNORECASE | re.MULTILINE)[1].strip()
                    elif re.match(r'^```', cleaned_text):
                        cleaned_text = re.split(r'^```[\w]*\s*|\s*```$', cleaned_text, flags=re.MULTILINE)[1].strip()
                ...
                return {
                    "text": text,
                    "parsed": parsed,                    # ← structured result if expect_json=True
                ...
                if attempt < self.max_retries and self._is_retryable_error(e):
                    ...
                    total_backoff_ms += int(backoff_sec * 1000)
                    time.sleep(backoff_sec)
                    continue

                error_str = str(e).lower()
                if "context" in error_str and ("length" in error_str or "too long" in error_str) and not overflow_handled:
